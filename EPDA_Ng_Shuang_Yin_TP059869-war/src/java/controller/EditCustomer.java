    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                String loggedInUsername = loggedInCustomer.getId(); // Get the logged-in username

                if (action.equalsIgnoreCase("Edit")) {
                    Customer found_customer = customerFacade.find(loggedInUsername);
                    ...
                    request.setAttribute("successMessage", "Customer profile has been edited successfully!");
                    
                    // Update the loggedInCustomer object in the session with the edited values
                    loggedInCustomer.setPassword(found_customer.getPassword());
                    loggedInCustomer.setPhone_number(found_customer.getPhone_number());
                    loggedInCustomer.setGender(found_customer.getGender());
                    loggedInCustomer.setDate_of_birth(found_customer.getDate_of_birth()); 
                    loggedInCustomer.setBalance(found_customer.getBalance()); 
                ...
                // If there is an error, forward the request to edit_customer.jsp...
                request.getRequestDispatcher("edit_customer.jsp").forward(request, response);
