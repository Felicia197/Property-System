 ...
 * @author Felicia
 */
@WebServlet(name = "RentRateOwner", urlPatterns = {"/RentRateOwner"})
public class RentRateOwner extends HttpServlet {
            ...
            try {
                ...
                if(action != null){
                    ...
                    if(action.equalsIgnoreCase("Rate Property")){
                        ...
                        // Check if property exists (Rent)
                        RentProperty found_rent_property = rentPropertyFacade.find(property_name);
                        ...
                        if (found_rent_property.getProperty_status().equalsIgnoreCase("Available")){
                            request.setAttribute("errorMessage", "Property is not rented");
                            throw new Exception();
                        }
                        
                        
                        // Check if the property belongs to the owner (Rent)
                ...
                String rentstatusToSearch = "Not Available";
                ArrayList<RentProperty> availablerentProperties = new ArrayList<>();

                for (RentProperty rentProperty : allrentProperties) {
                    if (rentProperty.getProperty_status().equalsIgnoreCase(rentstatusToSearch) &&
                        rentProperty.getOwner().equalsIgnoreCase(loggedInUsername)) {
                        availablerentProperties.add(rentProperty);
                    }
                }
 
                // Set the properties attribute in the request object
                request.setAttribute("rentproperties", availablerentProperties); 

                request.getRequestDispatcher("rent_rating_owner.jsp").forward(request, response);     
                
            }
            
            catch (Exception e){
                request.getRequestDispatcher("rent_rating_owner.jsp").include(request, response);            
            }        
        }
    }
